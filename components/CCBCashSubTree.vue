<template>
	...
	<view class="sonV" :style="{'borderColor': (subTreeData.investType ==  0) ? '#4473FF': '#41D380'}">
		...
		<view style="display: flex;">
			...
			<image class="imgIcon"  @click="openClick()"  style="margin-left: 8px;" mode="aspectFit" :src="bindImgSrc()"> </image>
		...
		<view>
			...
			<view class="upView" style="margin-top: 2px;  height: 20px;">
				...
				<view class="cellView" v-for="(tagItem, index) in bindTag(subTreeData.label)" :key="bindKey(index)"
	...
	export default {
		...
		methods: {
			...
			openClick(){
			    ...
			    this.subTreeData.isOpen = !this.subTreeData.isOpen;	
				
			
			},
			bindImgSrc(){
				console.log('subtree.invests = ' + JSON.stringify(this.subTreeData.invests));
				if(this.subTreeData.invests.length > 0){
					
					if(!this.subTreeData.isOpen){
						
						return  require("./jia_icon.png");
						
					}
					else{
						return  require("./jian_icon.png");
